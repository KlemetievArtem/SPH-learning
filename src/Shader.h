...
private:
	...
	std::string loadShaderSource(const std::string& filePath) {
		...
		if (in_file.is_open()) {
			...
			std::cout << "ERROR::SHADER::COULD_NOT_OPEN_FILE: "<< filePath <<"\n";
		}
		in_file.close();
	...
	GLuint loadShader(GLenum type, const std::string& filePath) {
		...
		if (!success) {
			...
			std::cout << "ERROR::SHADER::COULD_NOT_COMPILE_FILE: " << filePath << "\n";
			std::cout << infoLog << "\n";
		}
		return shader;
	}
...
public:
	...
	Shader(const int versionMajor, const int versionMinor, const std::string& vertexFile, const std::string& fragmentFile, const std::string& geometryFile = "")
